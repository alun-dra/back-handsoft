...
func (_q *CityQuery) QueryRegion() *RegionQuery {
...
// QueryCommunes chains the current query on the "communes" edge.
func (_q *CityQuery) QueryCommunes() *CommuneQuery {
...
func (_q *CityQuery) Clone() *CityQuery {
	...
	return &CityQuery{
		...
		withCommunes: _q.withCommunes.Clone(),
...
func (_q *CityQuery) WithRegion(opts ...func(*RegionQuery)) *CityQuery {
	query := (&RegionClient{config: _q.config}).Query()
...
// WithCommunes tells the query-builder to eager-load the nodes that are connected to
// the "communes" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *CityQuery) WithCommunes(opts ...func(*CommuneQuery)) *CityQuery {
	query := (&CommuneClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withCommunes = query
	return _q
}
...
func (_q *CityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*City, error) {
	...
	if query := _q.withCommunes; query != nil {
		if err := _q.loadCommunes(ctx, query, nodes,
			func(n *City) { n.Edges.Communes = []*Commune{} },
			func(n *City, e *Commune) { n.Edges.Communes = append(n.Edges.Communes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CityQuery) loadRegion(ctx context.Context, query *RegionQuery, nodes []*City, init func(*City), assign func(*City, *Region)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*City)
	for i := range nodes {
		if nodes[i].region_cities == nil {
			continue
		}
		fk := *nodes[i].region_cities
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "region_cities" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *CityQuery) loadCommunes(ctx context.Context, query *CommuneQuery, nodes []*City, init func(*City), assign func(*City, *Commune)) error {
	...
	query.withFKs = true
	query.Where(predicate.Commune(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(city.CommunesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.city_communes
		if fk == nil {
			return fmt.Errorf(`foreign-key "city_communes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "city_communes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
