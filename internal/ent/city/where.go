...
// HasCommunes applies the HasEdge predicate on the "communes" edge.
func HasCommunes() predicate.City {
	return predicate.City(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasCommunesWith applies the HasEdge predicate on the "communes" edge with a given conditions (other predicates).
func HasCommunesWith(preds ...predicate.Commune) predicate.City {
	return predicate.City(func(s *sql.Selector) {
		step := newCommunesStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
