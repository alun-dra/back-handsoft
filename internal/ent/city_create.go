...
func (_c *CityCreate) SetRegionID(id int) *CityCreate {
	_c.mutation.SetRegionID(id)
	return _c
}

// SetRegion sets the "region" edge to the Region entity.
func (_c *CityCreate) SetRegion(v *Region) *CityCreate {
	return _c.SetRegionID(v.ID)
}

// AddCommuneIDs adds the "communes" edge to the Commune entity by IDs.
func (_c *CityCreate) AddCommuneIDs(ids ...int) *CityCreate {
	_c.mutation.AddCommuneIDs(ids...)
	return _c
}

// AddCommunes adds the "communes" edges to the Commune entity.
func (_c *CityCreate) AddCommunes(v ...*Commune) *CityCreate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddCommuneIDs(ids...)
}
...
func (_c *CityCreate) check() error {
	...
	if len(_c.mutation.RegionIDs()) == 0 {
		return &ValidationError{Name: "region", err: errors.New(`ent: missing required edge "City.region"`)}
	}
	return nil
}
...
func (_c *CityCreate) createSpec() (*City, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.RegionIDs(); len(nodes) > 0 {
		...
		_node.region_cities = &nodes[0]
		_spec.Edges = append(_spec.Edges, edge)
	}
	if nodes := _c.mutation.CommunesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
